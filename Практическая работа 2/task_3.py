...
def get_mean_green_index(list_green_index): 

# считаем количество индексов
    counter = 0
    for territory_name in list_territories:
        counter = counter + 1

    mean_green_index = round(sum(list_green_index) / counter, 2)
    return mean_green_index

# распечатать результаты метода
print(get_mean_green_index(list_green_index))
