# размещаем части кода по поиску индекса озеленения и созданию нового списка
...
for territory in list_territories:
    gzi_v2 = GreenZoneIndex(territory_name = territory["territory_name"], territory_area = territory["territory_area"], green_zones = territory["green_zones"])
    value = gzi_v2.green_index
    list_green_index.append(value)


# сделать список - число территорий, индекс озеленения которых ниже заданного минимального порога
def filter_min_green_index(list_green_index, min_green_index = 0.1):
    """
    :param min_green_index: Минимальное значение индекса
    """

    list_green_index_min = []

    counter = 0
    for index in list_green_index:
        if index < min_green_index:
            counter = counter + 1       
    return counter
    
print(filter_min_green_index(list_green_index))